...
def home_page():
    if 'user' not in session:
        ...
        doctor = [doctor.to_dict() for doctor in db.collection(u'page').stream()]
        return render_template('index.html',page=page,doctor =doctor)
    else:
...
def login():
    if request.method == 'GET':
        ...
        if flag_4 == False:
            flash(
                'Incorrect, unverified or non-existent e-mail, division or password...', 'error')
...
def admin():
    if request.method == 'GET':
        ...
        if admin_user['email'] == 'admindokonsulta@gmail.com':
...
def admin_add():
    if request.method == 'GET':
        ...
        db.collection('doctor').document(email.lower()).set({
            'firstName': fname.lower(),
            'lastName': lname.lower(),
            'middleName': mname.lower(),
            'gender': gender.lower(),
